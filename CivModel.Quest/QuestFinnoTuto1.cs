...
{
    public class QuestFinnoTuto1 : Quest, ITileObjectObserver, IBattleObserver, IActorObserver
    {
        private const string Move = "move";
        private const string Attack = "attack";
        private const string Hold = "hold";

        public QuestFinnoTuto1(Game game)
            : base(game.GetPlayerFinno(), game.GetPlayerFinno(), typeof(QuestFinnoTuto1))
        {
        }
        ...
        protected override void OnComplete()
        {
            if (Requestee.Cities.FirstOrDefault() is CityBase city)
            {
                ...
                new Finno.JediKnight(Requestee, adj).OnAfterProduce(null);
            }

            var quest = Requestee.Quests.OfType<QuestFinnoTuto2>().FirstOrDefault();
            if (quest != null)
            {
                quest.Deploy();
                quest.Accept();
